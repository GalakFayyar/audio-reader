...
function processImageDir(path) {
    fs.readdir(musicDirectory + path, function(err, files) {
        console.log(files);
        var _tracks = [];
...
/*
processImageDir('./public/music/', "/public/albums.json", function(message) {
  getDirs('./public/music/', )
    console.log(message);
});
*/
/*
...
*/
var musicDirectory = './public/music/';
getDirs(musicDirectory, function(albums) {
    console.log(albums);
    albums.forEach(function(album) {
        processImageDir(album, function(message) {
            console.log(message);
        })
    })

    setTimeout(function() {
        fs.writeFile(__dirname + '/' + "public/albums.json", JSON.stringify(listAlbums), function(err) {
            if (err) throw err;
            console.log(JSON.stringify(listAlbums));
        });
    }, 3000);
});
